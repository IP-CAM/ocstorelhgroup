...
class ControllerModuleJournal2Popup extends Controller {
    ...
    public function index($setting) {
        ...
        if ($cache === null || self::$CACHEABLE !== true) {
            ...
            $this->data['information_id'] = $this->informationId();

			if ($this->data['information_id']) {
    ...
    public function show() {
    	if (isset($this->request->get['privacy'])) {
			$this->journal2->settings->set('popup_privacy_information', $this->request->get['privacy']);
		}
    ...
    private function validate() {
		...
		if ($information_info && (!isset($this->request->post['agree']) || !$this->request->post['agree'])) {
			$this->contact_error['agree'] = sprintf($this->language->get('error_agree'), $information_info['title']);
		}
	...
	private function informationId() {
		if (!$this->journal2->settings->get('popup_privacy', '1')) {
			return 0;
		}

		return (int)$this->journal2->settings->get('popup_privacy_information');
	}
